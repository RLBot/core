...
public static class ConfigParser
{
    ...
    private static T GetValue<T>(this TomlTable table, string key, T fallback, List<string> missingValues)
    {
        if (table.TryGetValue(key, out var res)) return (T)res;
        missingValues.Add(key);
        return fallback;
    }

    private static T GetEnum<T>(this TomlTable table, string key, T fallback, List<string> missingValues)
        where T : struct, Enum
    {
        if (table.TryGetValue(key, out var val))
        {
            if (Enum.TryParse((string)val, true, out T res)) return res;
            Logger.LogError($"Invalid value '{val}' for key '{key}'. Using default instead: {fallback}.");
            return fallback;
        }
    ...
    {
        string runCommandWindows = runnableSettings.GetValue<string>("run_command", "", missingValues);

        if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            return runCommandWindows;

        return runnableSettings.GetValue("run_command_linux", runCommandWindows, missingValues);
    }
    ...
    private static uint GetTeam(TomlTable table, List<string> missingValues)
    {
        ...
        switch (raw)
        {
            case long i and >= 0 and <= 1:  // Toml numbers are longs by default
                return (uint)i;
            ...
            case string s when s.Equals("orange", StringComparison.OrdinalIgnoreCase):
                return 1;
            default:
                Logger.LogError($"'team' has invalid value '{raw}'. Using team blue (0) instead.");
                return 0;
        }
    }
    ...
    )
    {
        ...
        (PlayerClassUnion variety, bool useConfig) = playerClass switch
        {
            ...
            PlayerClass.Psyonix => (PlayerClassUnion.FromPsyonix(
                new PsyonixT
                {
                    BotSkill = table.GetEnum(
                        "skill",
        ...
        {
            Logger.LogError(
                $"A car with type 'rlbot' is missing the 'config' field in {Path.GetFileName(matchConfigPath)}.");
        }
        
        if (useConfig && configPath != "")
        {
            string absoluteConfigPath = Path.Combine(matchConfigDir, configPath);
            player = LoadPlayerConfig(absoluteConfigPath, variety, team, nameOverride, loadoutFileOverride,
                missingValues);
        }
        else
        {
    ...
    private static PlayerConfigurationT LoadPlayerConfig(string configPath, PlayerClassUnion variety, uint team,
        string? nameOverride, string? loadoutFileOverride, List<string> missingValues)
    {
    ...
    )
    {
        ...
        TomlTable teamLoadout = loadoutToml.GetValue<TomlTable>(teamLoadoutString, [], missingValues);
        ...
        return new PlayerLoadoutT()
        {
            ...
            CustomColorId = (uint)teamLoadout.GetValue<long>("custom_color_id", 0, missingValues),
            ...
            PaintFinishId = (uint)teamLoadout.GetValue<long>("paint_finish_id", 0, missingValues),
            CustomFinishId = (uint)teamLoadout.GetValue<long>("custom_finish_id", 0, missingValues),
            EngineAudioId = (uint)teamLoadout.GetValue<long>("engine_audio_id", 0, missingValues),
            TrailsId = (uint)teamLoadout.GetValue<long>("trails_id", 0, missingValues),
            GoalExplosionId = (uint)teamLoadout.GetValue<long>("goal_explosion_id", 0, missingValues),
            LoadoutPaint = new LoadoutPaintT
            {
                CarPaintId = (uint)teamPaint.GetValue<long>("car_paint_id", 0, missingValues),
                DecalPaintId = (uint)teamPaint.GetValue<long>("decal_paint_id", 0, missingValues),
                WheelsPaintId = (uint)teamPaint.GetValue<long>("wheels_paint_id", 0, missingValues),
                BoostPaintId = (uint)teamPaint.GetValue<long>("boost_paint_id", 0, missingValues),
                AntennaPaintId = (uint)teamPaint.GetValue<long>("antenna_paint_id", 0, missingValues),
                HatPaintId = (uint)teamPaint.GetValue<long>("hat_paint_id", 0, missingValues),
                TrailsPaintId = (uint)teamPaint.GetValue<long>("trails_paint_id", 0, missingValues),
                GoalExplosionPaintId = (uint)teamPaint.GetValue<long>("goal_explosion_paint_id", 0, missingValues),
            },
    ...
    private static MutatorSettingsT GetMutatorSettings(
        ...
        new MutatorSettingsT
        {
            MatchLength = mutatorTable.GetEnum(
                "match_length", MatchLengthMutator.FiveMinutes, missingValues
            ),
            MaxScore = mutatorTable.GetEnum(
                "max_score", MaxScoreMutator.Default, missingValues
            ),
            MultiBall = mutatorTable.GetEnum(
                "multi_ball", MultiBallMutator.One, missingValues
            ),
            Overtime = mutatorTable.GetEnum(
                "overtime", OvertimeMutator.Unlimited, missingValues
            ),
            GameSpeed = mutatorTable.GetEnum(
                "game_speed", GameSpeedMutator.Default, missingValues
            ),
            BallMaxSpeed = mutatorTable.GetEnum(
                "ball_max_speed", BallMaxSpeedMutator.Default, missingValues
            ),
            BallType = mutatorTable.GetEnum(
                "ball_type", BallTypeMutator.Default, missingValues
            ),
            BallWeight = mutatorTable.GetEnum(
                "ball_weight", BallWeightMutator.Default, missingValues
            ),
            BallSize = mutatorTable.GetEnum(
                "ball_size", BallSizeMutator.Default, missingValues
            ),
            BallBounciness = mutatorTable.GetEnum(
                "ball_bounciness", BallBouncinessMutator.Default, missingValues
            ),
            Boost = mutatorTable.GetEnum(
                "boost_amount", BoostMutator.NormalBoost, missingValues
            ),
            Rumble = mutatorTable.GetEnum(
                "rumble", RumbleMutator.NoRumble, missingValues
            ),
            BoostStrength = mutatorTable.GetEnum(
                "boost_strength", BoostStrengthMutator.One, missingValues
            ),
            Gravity = mutatorTable.GetEnum(
                "gravity", GravityMutator.Default, missingValues
            ),
            Demolish = mutatorTable.GetEnum(
                "demolish", DemolishMutator.Default, missingValues
            ),
            RespawnTime = mutatorTable.GetEnum(
                "respawn_time", RespawnTimeMutator.ThreeSeconds, missingValues
            ),
            MaxTime = mutatorTable.GetEnum(
                "max_time", MaxTimeMutator.Default, missingValues
            ),
            GameEvent = mutatorTable.GetEnum(
                "game_event", GameEventMutator.Default, missingValues
            ),
            Audio = mutatorTable.GetEnum(
                "audio", AudioMutator.Default, missingValues
            ),
        };
    ...
    public static MatchConfigurationT LoadMatchConfig(string path)
    {
        ...
        TomlTable mutatorTable = rlbotToml.GetValue<TomlTable>("mutators", [], missingValues);

        TomlTableArray players = rlbotToml.GetValue<TomlTableArray>("cars", [], missingValues);
        ...
        TomlTableArray scripts = rlbotToml.GetValue<TomlTableArray>("scripts", [], missingValues);
        ...
        foreach (var scriptTable in scripts)
        {
            ...
            if (configPath != "")
            {
                scriptConfigs.Add(LoadScriptConfig(path, missingValues));
            }
            else
            {
                Logger.LogError($"A script is missing the 'config' field in {Path.GetFileName(path)}.");
            }
        }
